...
export default function Suppliers() {
  ...
  const { data: suppliers } = trpc.supplier.list.useQuery({ search: search || undefined });
  const utils = trpc.useUtils();
  const createMut = trpc.supplier.create.useMutation({ onSuccess: () => { utils.supplier.list.invalidate(); setShowForm(false); } });
  const updateMut = trpc.supplier.update.useMutation({ onSuccess: () => { utils.supplier.list.invalidate(); setShowForm(false); setEditing(null); } });
  const deleteMut = trpc.supplier.delete.useMutation({ onSuccess: () => utils.supplier.list.invalidate() });
    ...
    <div className="space-y-4">
          ...
          <Card key={s.id} className="hover:shadow-lg transition-all">
            <CardContent className="p-4">
              <div className="flex items-start gap-4">
                {s.image ? <img src={s.image} alt={s.name} className="w-16 h-16 rounded-lg object-cover" /> : (
                  <div className="w-16 h-16 rounded-lg bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center text-white"><Truck className="w-8 h-8" /></div>
                )}
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-mono text-muted-foreground">{s.rnc}</p>
                </div>
                <div className="flex flex-col gap-1">
                  <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => { setEditing(s); setForm(s); setShowForm(true); }}><Pencil className="w-3 h-3" /></Button>
                  <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-500" onClick={() => deleteMut.mutate({ id: s.id })}><Trash2 className="w-3 h-3" /></Button>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editing ? t("editSupplier") : t("addSupplier")}</DialogTitle></DialogHeader>
          <form onSubmit={(e) => { e.preventDefault(); if (editing) updateMut.mutate({ id: editing.id, ...form }); else createMut.mutate(form); }} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2"><Label>{t("contactName")}</Label><Input value={form.contactName || ""} onChange={(e) => setForm({ ...form, contactName: e.target.value })} /></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2"><Label>{t("rnc")}</Label><Input value={form.rnc || ""} onChange={(e) => setForm({ ...form, rnc: e.target.value })} /></div>
